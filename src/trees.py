...
def tree_from_str(treebank, strip_top=False, strip_spmrl_features=False):
    trees = load_trees(None,
        strip_top=strip_top,
        strip_spmrl_features=strip_spmrl_features,
        treebank=treebank)

    assert len(trees) == 1
    return trees[0]

def load_trees(path, strip_top=False, strip_spmrl_features=False, treebank=None):
